...
class OpenCenterTestCase(unittest2.TestCase):
    """
    This test case assumes a opencenter-server has been successfully created,
    and at least 3 nodes have opencenter-agent installed.
    """
    @classmethod
    def setUpClass(self):
        pass
    ...
    def setUp(self):
        ...
        if self.user:
            self.ep = OpenCenterEndpoint(
                self.endpoint_url, user=self.user, password=self.password)
        else:
            self.ep = OpenCenterEndpoint(self.endpoint_url)

        self.admin_ep = OpenCenterEndpoint(
            self.endpoint_url + '/admin',
            user=self.user,
    ...
    def test_happy_path(self):
        ...
        # resp = self._post_new_plan(plan.execution_plan.raw_plan,
        # self.workspace)
        # self.assertEquals(resp.status_code, 202)
        # task = resp.task
        # task.wait_for_complete()
        ...
        infra_container = self.ep.nodes.filter(
        ...
        self.assertEquals(compute_container.facts[
                          'parent_id'], test_cluster.id)
        az_container = self.ep.nodes.filter('name = "AZ nova"').first()
        self.assertIsNotNone(az_container)
        self.assertEquals(az_container.facts[
                          'parent_id'], compute_container.id)
